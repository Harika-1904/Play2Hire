...
export default function RecruiterDashboard() {
  ...
  // ✅ FIXED SEND INVITE
  const sendInvite = async (learner) => {
  ...
  await fetch("http://localhost:5000/api/invites/send", {
    ...
    body: JSON.stringify({
      ...
      learnerEmail: learnerEmail,   // ✅ FIX
      learnerName: learner.name,
      message: "You are shortlisted 🚀",
    }),
  });

  localStorage.setItem("inviteUpdated", Date.now());
  alert("Invite Sent ✅");
};
  return (
    <>
          ...
          {learners.map((l, i) => {
            ...
            return (
              ...
              >
                ...
                {/* Progress */}
                <div style={{ backgroundColor: "#ddd", height: "15px", marginTop: "5px" }}>
                  <div
                    style={{
                      width: `${Math.min(score, 100)}%`,
                      backgroundColor:
                        score >= 80 ? "gold" : score >= 50 ? "lightgreen" : "red",
                      height: "15px",
                    }}
                  ></div>
                </div>
                ...
                <button
                  ...
                  onClick={() => sendInvite(l)}   // 🔥 IMPORTANT FIX
                >
                  Send Invite 📩
                </button>
              </div>
